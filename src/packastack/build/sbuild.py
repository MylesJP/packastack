...
class SbuildResult:
    ...
    # Per-interpreter results when an explicit Python matrix was requested.
    python_version_results: dict[str, SbuildResult] = field(default_factory=dict)
...
class SbuildConfig:
    ...
    chroot_name: str = ""
    # sbuild chroot backend: "schroot" (directory chroots managed by
    # schroot) or "unshare" (rootless tarball chroots, sbuild >= 0.88
    # default). Always passed explicitly so the host's sbuild default
    ...
    # Explicit pybuild interpreter matrix.  Each version is installed as an
    # sbuild-only build-dependency and selected with PYBUILD_VERSIONS.
    python_versions: list[str] = field(default_factory=list)

    def __post_init__(self) -> None:
        self.python_versions = normalize_python_versions(self.python_versions)


def normalize_python_versions(versions: object) -> list[str]:
    """Validate and normalise an explicit Python version matrix.

    Version strings are deliberately narrow: they become both Debian package
    names (``python<version>``) and values passed to ``PYBUILD_VERSIONS``.
    """
    if versions is None:
        return []
    if not isinstance(versions, list):
        raise ValueError("sbuild.python_versions must be a list of strings")

    normalized: list[str] = []
    for version in versions:
        if not isinstance(version, str) or not _PYTHON_VERSION_RE.fullmatch(version):
            raise ValueError("sbuild.python_versions entries must look like '3.14'")
...
def _perl_quote(value: str) -> str:
    """Escape a string for interpolation into a single-quoted Perl literal."""
    return value.replace("\\", "\\\\").replace("'", "\\'")


def write_sbuild_config_fragment(
    log_dir: Path,
    python_versions: list[str],
    bind_mounts: list[tuple[str, str]] | None = None,
) -> Path | None:
    """Write an additive sbuild config fragment for this build.

    ``SBUILD_CONFIG`` is read on the host before sbuild enters the chroot. The
    fragment remains beside the build logs so the effective settings are
    auditable. An existing user-supplied ``SBUILD_CONFIG`` is loaded first and
    preserved.

    For a Python build matrix: ``DEBPYTHON3_SUPPORTED`` makes Debian helpers
    such as ``py3versions`` and ``pkgos-dh_auto_test`` see the complete
    matrix. ``PYBUILD_VERSIONS`` covers direct pybuild use, while ``PYTHON``
    selects the newest configured version for custom stestr commands that
    bypass the version-aware helpers.

    For unshare-mode builds: ``$unshare_bind_mounts`` makes host directories
    (the PackaStack local APT repo) visible inside the chroot, replacing the
    schroot profile fstab used in schroot mode.

    Args:
        log_dir: Directory where the fragment is written.
        python_versions: Explicit pybuild interpreter matrix (may be empty).
        bind_mounts: (host_directory, chroot_mountpoint) pairs for
            unshare-mode builds (may be None/empty).

    Returns:
        Path to the fragment, or None when there is nothing to configure.
    """
    if not python_versions and not bind_mounts:
        return None

    path = log_dir / "sbuild-packastack.conf"
    previous_config = os.environ.get("SBUILD_CONFIG")
    lines = ["# Generated by Packastack; retained with this build's logs."]
    if previous_config:
        lines.append(
            f"do '{_perl_quote(previous_config)}' "
            'or die "failed to load $ENV{SBUILD_CONFIG}: $@";'
        )
    if bind_mounts:
        entries = ", ".join(
            f"{{ directory => '{_perl_quote(directory)}', "
            f"mountpoint => '{_perl_quote(mountpoint)}' }}"
            for directory, mountpoint in bind_mounts
        )
        lines.append(f"$unshare_bind_mounts = [ {entries} ];")
    if python_versions:
        lines.append(
            f"$build_environment->{{'DEBPYTHON3_SUPPORTED'}} = '{','.join(python_versions)}';"
        )
        lines.append(
            f"$build_environment->{{'PYBUILD_VERSIONS'}} = '{' '.join(python_versions)}';"
        )
        lines.append(f"$build_environment->{{'PYTHON'}} = 'python{python_versions[-1]}';")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def build_sbuild_command(config: SbuildConfig) -> list[str]:
    ...
    # The chroot mode is always explicit: sbuild >= 0.88 defaults to
    # unshare, and a schroot name passed to an unshare-mode sbuild (or
    # vice versa) is unresolvable.
    cmd = ["sbuild", "--nolog", f"--chroot-mode={config.chroot_mode}"]
    ...
    # in .changes files and -c to pick which chroot to enter. In schroot
    # mode the name is a registered schroot; in unshare mode it is the
    # path to (or cache name of) the build tarball.
    if config.chroot_name:
        cmd.extend(["-c", config.chroot_name])

    # Local repo setup — ensure indexes exist, make the repo visible
    # inside the chroot, and add apt source commands to the chroot.
    #
    # How the repo gets into the chroot depends on the mode: schroot uses
    # a bind-mount configured in the schroot profile fstab; unshare has no
    # fstab, so the bind-mount is injected via the $unshare_bind_mounts
    # option in the SBUILD_CONFIG fragment written by _run_sbuild_once.
    if config.local_repo_root and config.local_repo_root.exists():
        ensure_repo_initialized(config.local_repo_root, config.arch)

        repo_ready = False
        if config.chroot_mode == "unshare":
            repo_ready = True
        elif config.chroot_name:
            try:
        ...
        if repo_ready:
            ...
            # Cleanup only matters for schroot sessions; an unshare
            # session is a throwaway extraction of the tarball.
            if config.chroot_mode != "unshare":
    ...
    # sbuild's manual dependencies are intentionally ephemeral: they install
    # the requested interpreters without changing the source package's
    # debian/control.  PYBUILD_VERSIONS is supplied separately at execution.
    for version in config.python_versions:
        cmd.append(f"--add-depends=python{version}")
...
def _run_sbuild_once(config: SbuildConfig, timeout: int = 3600) -> SbuildResult:
    """Run one sbuild pass.
    ...
    # Unshare mode has no schroot fstab, so the local repo bind-mount is
    # injected through the sbuild config fragment instead.
    bind_mounts: list[tuple[str, str]] = []
    if (
        config.chroot_mode == "unshare"
        and config.local_repo_root
        and config.local_repo_root.exists()
    ):
        bind_mounts.append((str(config.local_repo_root.resolve()), CHROOT_REPO_MOUNT))

    config_fragment = write_sbuild_config_fragment(log_dir, config.python_versions, bind_mounts)
    environment = None
    if config_fragment:
        environment = os.environ.copy()
        environment["SBUILD_CONFIG"] = str(config_fragment)
    ...
    try:
        ...
        return SbuildResult(
            ...
            python_versions_tested=(
                list(config.python_versions)
                if overall_success and config.python_versions
                else parse_pybuild_python_versions(combined_output)
            ),
        )
...
def _run_python_sbuild_matrix(config: SbuildConfig, timeout: int) -> SbuildResult:
    """Run an isolated sbuild pass for every explicitly configured interpreter."""
    output_dir = config.output_dir
    log_dir = config.run_log_dir or output_dir
    results: dict[str, SbuildResult] = {}

    for version in config.python_versions:
        pass_name = f"python{version}"
        pass_config = replace(
            config,
            output_dir=output_dir / pass_name,
            run_log_dir=log_dir / pass_name,
            python_versions=[version],
        )
        results[version] = _run_sbuild_once(pass_config, timeout)

    log_dir.mkdir(parents=True, exist_ok=True)
    combined_output = "".join(
        f"\n===== sbuild Python {version} pass =====\n{result.output}"
        for version, result in results.items()
    )
    stdout_log = log_dir / "sbuild.stdout.log"
    stderr_log = log_dir / "sbuild.stderr.log"
    stdout_log.write_text(combined_output, encoding="utf-8")
    stderr_log.write_text("", encoding="utf-8")

    primary_result = next(iter(results.values()))
    failed = [(version, result) for version, result in results.items() if not result.success]
    successful_versions = [version for version, result in results.items() if result.success]
    if failed:
        validation_message = "; ".join(
            f"Python {version}: {result.validation_message or 'sbuild failed'}"
            for version, result in failed
        )
        exit_code = failed[0][1].exit_code
    else:
        validation_message = (
            f"All configured Python sbuild passes succeeded: {', '.join(successful_versions)}"
        )
        exit_code = 0
    ...
    return SbuildResult(
        success=not failed,
        output=combined_output,
        # The first configured pass supplies the publishable artifacts. Each
        # pass has a separate output directory, so later validation builds do
        # not overwrite them.
        artifacts=primary_result.artifacts,
        changes_file=primary_result.changes_file,
        chroot_name=primary_result.chroot_name,
        setup_method=primary_result.setup_method,
        local_repo_path=primary_result.local_repo_path,
        exit_code=exit_code,
        collected_artifacts=primary_result.collected_artifacts,
        collected_logs=[item for result in results.values() for item in result.collected_logs],
        stdout_log_path=stdout_log,
        stderr_log_path=stderr_log,
        primary_log_path=stdout_log,
        searched_dirs=list(
            dict.fromkeys(item for result in results.values() for item in result.searched_dirs)
        ),
        validation_message=validation_message,
        report_path=primary_result.report_path,
        command=primary_result.command,
        python_versions_tested=successful_versions,
        python_version_results=results,
    )


def run_sbuild(config: SbuildConfig, timeout: int = 3600) -> SbuildResult:
    """Build once normally, or once per explicitly configured Python version."""
    ...
    if len(config.python_versions) > 1:
        return _run_python_sbuild_matrix(config, timeout)
    return _run_sbuild_once(config, timeout)
