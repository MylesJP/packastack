...
class TestBuildSbuildCommand:
    ...
    def test_chroot_mode_always_explicit(self, tmp_path: Path) -> None:
        """The schroot mode is pinned so the host sbuild default is ignored."""
        config = SbuildConfig(
            dsc_path=tmp_path / "pkg.dsc",
            output_dir=tmp_path,
            distribution="noble",
        ...
        cmd = build_sbuild_command(config)
        assert "--chroot-mode=schroot" in cmd

    def test_unshare_chroot_mode_flag(self, tmp_path: Path) -> None:
        """Unshare mode is passed through to the sbuild command line."""
        config = SbuildConfig(
            dsc_path=tmp_path / "pkg.dsc",
            output_dir=tmp_path,
            distribution="noble",
        ...
        cmd = build_sbuild_command(config)
        assert "--chroot-mode=unshare" in cmd

    def test_unshare_local_repo_skips_schroot_fstab(self, tmp_path: Path) -> None:
        """Unshare mode never touches the schroot fstab; the bind-mount is
        injected via the SBUILD_CONFIG fragment instead."""
        repo = tmp_path / "repo"
        repo.mkdir()
        config = SbuildConfig(
            dsc_path=tmp_path / "pkg.dsc",
            ...
            distribution="noble",
            local_repo_root=repo,
            chroot_name="/home/user/.cache/sbuild/noble-amd64.tar.zst",
            chroot_mode="unshare",
            proposed=False,
        )
        with patch("packastack.build.sbuild.configure_repo_mount") as mock_mount:
            cmd = build_sbuild_command(config)
        mock_mount.assert_not_called()
        assert "--chroot-setup-commands" in cmd
        # No cleanup — the unshare session is a throwaway tarball extraction
        assert "--finished-build-commands" not in cmd

    def test_unshare_local_repo_works_without_chroot_name(self, tmp_path: Path) -> None:
        """Unshare repo setup does not depend on a chroot name."""
        repo = tmp_path / "repo"
        repo.mkdir()
        config = SbuildConfig(
            dsc_path=tmp_path / "pkg.dsc",
...
class TestPythonVersionsSbuildConfig:
    ...
    def test_returns_none_for_an_empty_matrix(self, tmp_path: Path) -> None:
        assert write_sbuild_config_fragment(tmp_path, []) is None

    def test_returns_none_without_python_or_bind_mounts(self, tmp_path: Path) -> None:
        assert write_sbuild_config_fragment(tmp_path, [], []) is None

    def test_writes_unshare_bind_mounts(self, tmp_path: Path) -> None:
        path = write_sbuild_config_fragment(tmp_path, [], [("/host/repo", "/srv/packastack-apt")])
        assert path is not None
        assert path.read_text() == (
            "# Generated by Packastack; retained with this build's logs.\n"
            "$unshare_bind_mounts = [ { directory => '/host/repo', "
            "mountpoint => '/srv/packastack-apt' } ];\n"
        )

    def test_combines_bind_mounts_and_python_matrix(self, tmp_path: Path) -> None:
        path = write_sbuild_config_fragment(
            tmp_path, ["3.14"], [("/host/repo", "/srv/packastack-apt")]
        )
        assert path is not None
        content = path.read_text()
        assert "$unshare_bind_mounts" in content
        assert "$build_environment->{'PYBUILD_VERSIONS'} = '3.14';" in content

    def test_escapes_quotes_in_bind_mount_paths(self, tmp_path: Path) -> None:
        path = write_sbuild_config_fragment(
            tmp_path, [], [("/home/o'brien/repo", "/srv/packastack-apt")]
        )
        assert path is not None
        assert "directory => '/home/o\\'brien/repo'" in path.read_text()

    def test_preserves_existing_sbuild_config(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("SBUILD_CONFIG", "/home/user/.sbuildrc-extra")
        path = write_sbuild_config_fragment(tmp_path, ["3.14"])
        assert path is not None
        content = path.read_text()
        assert (
            "do '/home/user/.sbuildrc-extra' or die \"failed to load $ENV{SBUILD_CONFIG}: $@\";"
        ) in content

    def test_unshare_build_injects_bind_mount_config(self, tmp_path: Path) -> None:
        """An unshare-mode run exposes the local repo via SBUILD_CONFIG."""
        repo = tmp_path / "repo"
        repo.mkdir()
        config = SbuildConfig(
            dsc_path=tmp_path / "pkg.dsc",
        ...
        assert "--chroot-mode=unshare" in command
        env = mock_run.call_args.kwargs["env"]
        content = Path(env["SBUILD_CONFIG"]).read_text()
        assert "$unshare_bind_mounts" in content
