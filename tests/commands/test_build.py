...
class TestRunBuildPhases:
    ...
    def test_package_not_found_returns_config_error(
        ...
        with (
            ...
            patch.object(build, "load_project_releases", return_value=None),
        ):
...
class TestReleaseTarballFetch:
    ...
    def test_uscan_preferred_over_official(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build.GitFetcher, "fetch_and_checkout", return_value=mock_fetch_result))
            stack.enter_context(patch.object(build, "update_launchpad_yaml_series", return_value=(True, [], None)))
            stack.enter_context(patch.object(build, "select_upstream_source", return_value=mock_upstream))
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            # uscan returns a tarball; official should not be needed
            stack.enter_context(patch.object(build, "_run_uscan", return_value=(True, mock_tarball_path, "")))
            official_dl = stack.enter_context(patch.object(build, "download_and_verify_tarball"))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0", output="")))
            stack.enter_context(patch.object(build, "run_command"))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="1.0", epoch=0)))
            stack.enter_context(patch.object(build, "generate_release_version", return_value="1.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="Release"))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=MagicMock(success=True, needs_refresh=False, patch_reports=[])))
            stack.enter_context(patch.object(build, "build_source", return_value=MagicMock(success=True, artifacts=[], dsc_file=None, changes_file=None, output="")))
    ...
    def test_fallback_to_official_when_uscan_fails(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            # uscan fails
            stack.enter_context(patch.object(build, "_run_uscan", return_value=(False, None, "uscan failed")))
            # official succeeds
            official_dl = stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_tarball_result))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0", output="")))
            stack.enter_context(patch.object(build, "run_command"))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
    ...
    def test_github_fallback_when_official_fails(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            stack.enter_context(patch.object(build, "_run_uscan", return_value=(False, None, "uscan failed")))
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_official_result))
            stack.enter_context(patch.object(build, "_download_pypi_tarball", return_value=(False, None, "skip")))
            gh_dl = stack.enter_context(patch.object(build, "_download_github_release_tarball", return_value=(True, mock_tarball_path, "")))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0", output="")))
            stack.enter_context(patch.object(build, "run_command"))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
    ...
    def test_git_archive_fallback(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_official_result))
            stack.enter_context(patch.object(build, "_download_pypi_tarball", return_value=(False, None, "skip")))
            stack.enter_context(patch.object(build, "_download_github_release_tarball", return_value=(False, None, "skip")))
            stack.enter_context(patch.object(build, "run_command", return_value=MagicMock(returncode=0, output="")))
            git_archive = stack.enter_context(patch.object(build, "generate_snapshot_tarball", return_value=tarball_result))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0", output="")))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="1.0", epoch=0)))
    ...
    def test_milestone_uses_uscan_first(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            official_dl = stack.enter_context(patch.object(build, "download_and_verify_tarball"))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0b1", output="")))
            stack.enter_context(patch.object(build, "run_command"))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="1.0", epoch=0)))
            stack.enter_context(patch.object(build, "generate_milestone_version", return_value="1.0.0~b1-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="Milestone b1"))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=MagicMock(success=True, needs_refresh=False, patch_reports=[])))
            stack.enter_context(patch.object(build, "build_source", return_value=MagicMock(success=True, artifacts=[], dsc_file=None, changes_file=None, output="")))
    ...
    def test_pypi_fallback_when_official_fails(self, tmp_path: Path, mock_paths: dict, mock_run: MagicMock) -> None:
        ...
        with ExitStack() as stack:
            ...
            # official fails
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_official_result))
            pypi_dl = stack.enter_context(patch.object(build, "_download_pypi_tarball", return_value=(True, mock_tarball_path, "")))
            stack.enter_context(patch.object(build, "_download_github_release_tarball", return_value=(False, None, "skip")))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, created=False)))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, upstream_version="1.0.0", output="")))
            stack.enter_context(patch.object(build, "run_command"))
...
class TestFullBuildPhases:
    ...
    def test_successful_full_build(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "select_upstream_source", return_value=mock_upstream))
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_tarball_result))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="1.0")))
            stack.enter_context(patch.object(build, "generate_release_version", return_value="29.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="New release."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=mock_pq_result))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, branch_name="upstream-caracal", created=False, error="")))
            stack.enter_context(patch.object(build, "import_orig", return_value=MagicMock(success=True, output="", upstream_version="")))
            stack.enter_context(patch.object(build, "build_source", return_value=mock_build_result))
    ...
    def test_upstreamed_patches_block_without_force(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="New release."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[mock_upstreamed]))
    ...
    def test_no_upstream_source_returns_config_error(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build.GitFetcher, "fetch_and_checkout", return_value=mock_fetch_result))
            stack.enter_context(patch.object(build, "update_launchpad_yaml_series", return_value=(True, [], None)))
            stack.enter_context(patch.object(build, "select_upstream_source", return_value=None))
    ...
    def test_snapshot_build_no_tarball_url(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="Snapshot."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=mock_pq_result))
            stack.enter_context(patch.object(build, "ensure_upstream_branch", return_value=MagicMock(success=True, branch_name="upstream-caracal", created=False, error="")))
    ...
    def test_milestone_build(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "get_current_version", return_value="1.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="1.0")))
            stack.enter_context(patch.object(build, "generate_milestone_version", return_value="30.0.0.0~b1-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="Milestone b1."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=mock_pq_result))
...
class TestBuildWithUpstreamedPatchesForce:
    ...
    def test_upstreamed_patches_with_force(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_tarball_result))
            stack.enter_context(patch.object(build, "get_current_version", return_value="28.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="28.0.0")))
            stack.enter_context(patch.object(build, "generate_release_version", return_value="29.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="New release."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
...
class TestSnapshotAcquisitionIntegration:
    ...
    def test_snapshot_build_calls_acquire_upstream_snapshot(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "get_current_version", return_value="28.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=MagicMock(upstream="28.0.0")))
            stack.enter_context(patch.object(build, "generate_release_version", return_value="29.0.0.dev5-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="Snapshot."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=mock_pq_result))
    ...
    def test_snapshot_acquisition_failure_exits_early(
        ...
        with ExitStack() as stack:
            ...
            mock_build = stack.enter_context(
                patch.object(build, "build_source", return_value=MagicMock())
            )
...
class TestBuildVersionEdgeCases:
    ...
    def test_version_with_epoch(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(build, "apply_signature_policy", return_value=[]))
            stack.enter_context(patch.object(build, "download_and_verify_tarball", return_value=mock_tarball_result))
            stack.enter_context(patch.object(build, "get_current_version", return_value="1:28.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "parse_version", return_value=parsed_version))
            stack.enter_context(patch.object(build, "generate_release_version", return_value="1:29.0.0-0ubuntu1"))
            stack.enter_context(patch.object(build, "generate_changelog_message", return_value="New release."))
            stack.enter_context(patch.object(build, "update_changelog", return_value=True))
            stack.enter_context(patch.object(build, "check_upstreamed_patches", return_value=[]))
            stack.enter_context(patch.object(build, "pq_import", return_value=mock_pq_result))
    ...
    def test_validate_plan_only_exits_early(
        ...
        with ExitStack() as stack:
            ...
            mock_fetch = stack.enter_context(
                patch.object(build.GitFetcher, "fetch_and_checkout")
            )
